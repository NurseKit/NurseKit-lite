---
{"dg-publish":true,"permalink":"/medicaments/benzodiazepines/lorazepam/","tags":["médicament","benzodiazépine","anxiolytique","antiépileptique","urgences","prémédication"],"noteIcon":"","dg-note-properties":{"nom":"Lorazépam","classe":["Benzodiazépine","Anxiolytique","Antiépileptique (IV)","Prémédication"],"indications":["Anxiété sévère (court terme)","État de mal épileptique (IV → 1ère intention)","Prémédication anesthésique (amnésie antérograde ++)","Sevrage alcoolique","Agitation aiguë (IM/IV)"],"posologie":"PO : 1-2,5 mg x2-3/j. IV état de mal : 0,1 mg/kg (4 mg adulte) sur 2 min, renouvelable 1 fois. IM agitation : 1-2 mg. Prémédication : 2 mg la veille + 2 mg 2h avant","contre_indications":["Insuffisance respiratoire sévère non assistée","Myasthénie","SAOS non appareillé","Insuffisance hépatique sévère","Grossesse (fœtotoxique)"],"effets_secondaires":["Sédation, somnolence","Amnésie antérograde forte","Dépression respiratoire (IV)","Dépendance","Syndrome de sevrage (arrêt brutal)","Confusion, réaction paradoxale (sujet âgé)"],"service":["Urgences","Neurologie","Anesthésie","Psychiatrie"],"mécanisme":"Agoniste allostérique positif des récepteurs GABA-A (même mécanisme que diazépam). Demi-vie 10-20h, pas de métabolite actif (glucuronoconjugaison directe) → moins d'accumulation. Absorption IM fiable (avantage sur diazépam). Amnésie antérograde prononcée.","interactions":["Opioïdes IV → dépression respiratoire","Alcool → potentialisation","Autres dépresseurs SNC → sédation additive","Olanzapine IM → dépression respiratoire (délai 1h minimum entre les deux IM)"],"niveau_preuve":"A","tags":["médicament","benzodiazépine","anxiolytique","antiépileptique","urgences","prémédication"],"revision":"2026-06-22"}}
---

> [!abstract] En 2 mots
> La benzodiazépine de choix pour l'état de mal épileptique et la prémédication anesthésique → demi-vie intermédiaire, pas d'accumulation, pas de métabolite actif. Son fort effet amnésiant est un atout en prémédication (le patient ne se souviendra pas des gestes pré-opératoires) et un risque en anxiété chronique. Une interaction à ne jamais manquer : le délai d'une heure avec l'olanzapine IM.

---

## À quoi ça sert ?

Tu rencontreras le lorazépam dans des situations très variées :
- **État de mal épileptique** : 1ère intention en IV (0,1 mg/kg, la dose à retenir)
- **Anxiété sévère** (court terme ≤ 4 semaines)
- **Prémédication anesthésique** : fort effet amnésiant → le patient ne se souviendra probablement pas de la préparation
- **Agitation aiguë** psychiatrique : voie IM fiable (contrairement au diazépam IM)
- Sevrage alcoolique

---

## Comment ça marche ? (simplifié)

Même mécanisme que le diazépam : renforcement de l'inhibition GABA-A. Mais le lorazépam a trois avantages importants par rapport au diazépam :

1. **Demi-vie plus courte** (10-20h) → moins de sédation résiduelle le lendemain
2. **Pas de métabolite actif** : le foie l'élimine directement par glucuronoconjugaison → pas d'accumulation chez le sujet âgé ou l'insuffisant hépatique
3. **Absorption IM fiable** : c'est un avantage majeur → le diazépam IM est mal absorbé et imprévisible ; le lorazépam IM, lui, passe correctement

> Pense-y comme une version "plus propre" du diazépam → même efficacité antiépileptique, mais sans les métabolites qui traînent pendant des jours. C'est pour ça qu'il est préféré en état de mal et chez les patients fragiles.

---

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues et association dangereuse à connaître absolument
> - Insuffisance respiratoire sévère non assistée : risque d'apnée
> - **Lorazépam IM + Olanzapine IM = dépression respiratoire grave** → respecter un délai minimum d'**1 heure** entre les deux injections intramusculaires. Cette association est fréquente en psychiatrie pour l'agitation aiguë → c'est un piège qu'il faut connaître par cœur.

> [!warning] Précautions importantes
> - Arrêt progressif obligatoire (sevrage BZD) → comme toutes les BZD
> - Grossesse : éviter (fœtotoxique)
> - Personnes âgées : demi-dose, surveiller les chutes

> [!caution] À surveiller
> - SpO₂ et conscience lors de l'injection IV → en continu pendant et après l'injection
> - **Amnésie antérograde marquée** : prévenir le patient avant la prémédication → "vous ne vous souviendrez probablement pas de cette période" → c'est important pour la relation de confiance
> - Dépendance si traitement prolongé au long cours

---

## Ce que je fais en tant qu'IDE

- [ ] **IV état de mal** : injecter lentement (< 2 mg/min), monitorer SpO₂ et conscience en continu, flumazénil (Anexate®) disponible à portée de main
- [ ] **IM agitation** : injection au deltoïde ou fessier → fiable contrairement au diazépam IM
- [ ] **Prémédication** : avertir le patient que l'amnésie antérograde est un effet voulu → "vous n'aurez probablement aucun souvenir de la préparation, c'est normal et c'est prévu"
- [ ] **Association olanzapine IM** : vérifier qu'au moins 1h s'est écoulée depuis la dernière injection d'olanzapine avant d'injecter le lorazépam IM
- [ ] Déconseiller conduite et machines après prise

---

## Les 3 choses à retenir

1. **"État de mal = 0,1 mg/kg IV"** (4 mg chez l'adulte standard) en 2 minutes → 1ère intention, dose à avoir en tête
2. **"Avantage vs diazépam : pas de métabolite, IM fiable"** → préféré chez le sujet âgé, l'insuffisant hépatique, et en IM pour l'agitation
3. **"Lorazépam IM + Olanzapine IM = 1h de délai minimum"** → dépression respiratoire documentée si les deux sont injectés sans délai

---

## Voir aussi

[[Médicaments/Benzodiazépines/Diazepam\|Diazépam]] · [[Médicaments/Antidotes/Flumazenil\|Flumazénil]] · [[Médicaments/Antipsychotiques/Olanzapine\|Olanzapine (interaction IM)]]

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
