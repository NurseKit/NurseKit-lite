---
{"dg-publish":true,"permalink":"/medicaments/antiseptiques/chlorhexidine/","tags":["médicament","antiseptique","chlorhexidine","cathéter","dialyse-péritonéale"],"noteIcon":"","dg-note-properties":{"nom":"Chlorhexidine","classe":["Antiseptique","Biguanide"],"indications":["Antisepsie de la peau avant geste invasif (VVP, ponction, chirurgie)","Soins des cathéters (site de sortie)","Dialyse péritonéale (désinfection connecteur)","Soins de bouche en réanimation (prévention PAVM)"],"posologie":"Solution à 0,5-2 % pour la peau, 0,12 % pour les bains de bouche. Ne pas diluer les solutions prêtes à l'emploi.","contre_indications":["Nouveau-nés < 1 mois (toxicité cutanée)","Contact avec les méninges, l'oreille moyenne, les yeux","Allergie à la chlorhexidine (CI absolue → anaphylaxie possible)"],"effets_secondaires":["Réaction allergique (urticaire, anaphylaxie → rare mais grave)","Irritation cutanée locale","Coloration brunâtre des dents (bains de bouche au long cours)"],"service":["Tous services","Réanimation","Néphrologie"],"mécanisme":"Détruit la membrane cellulaire bactérienne → bactéricide à large spectre (Gram+, Gram−, champignons, virus enveloppés). Action rapide et rémanente (3-6h sur la peau).","interactions":["Incompatible avec les savons et les composés anioniques (ne pas mélanger avec Bétadine)"],"niveau_preuve":"A","tags":["médicament","antiseptique","chlorhexidine","cathéter","dialyse-péritonéale"],"revision":"2026-06-22"}}
---

> [!abstract] En 2 mots
> L'antiseptique de référence pour la peau avant tout geste invasif. Action large spectre + rémanente (elle continue d'agir 3-6h après application). Incompatible avec la Bétadine → jamais les deux ensemble.

---

## À quoi ça sert ?

La chlorhexidine est l'antiseptique cutané le plus utilisé en pratique infirmière. Tu la retrouves partout : avant la pose d'une VVP, pour les soins de cathéter, en dialyse péritonéale pour la désinfection du connecteur, et même en bains de bouche en réa pour prévenir les pneumonies sous ventilation mécanique (PAVM).

---

## Comment ça marche ? (simplifié)

Elle s'attaque directement à la membrane des bactéries et la détruit → comme crever un ballon. Et en plus, elle reste active sur la peau pendant plusieurs heures après l'application (effet rémanent), ce que la Bétadine ne fait pas.

> Un point clé : la Bétadine agit plus vite mais moins longtemps ; la chlorhexidine agit un peu plus lentement mais persiste. C'est pour ça qu'on la préfère pour les cathéters.

---

## Ce qu'il ne faut PAS faire

> [!danger] Allergie à la chlorhexidine = CI absolue
> Les chocs anaphylactiques à la chlorhexidine existent → ils sont rares mais graves. Toujours demander les allergies avant. Si un patient dit "j'ai eu une réaction lors d'un soin", explorer ça sérieusement.

> [!danger] Jamais sur les muqueuses sensibles
> - Pas dans les yeux (risque d'ulcère cornéen)
> - Pas dans l'oreille (ototoxique si contact avec l'oreille moyenne)
> - Pas sur les méninges
> - Nouveau-nés < 1 mois : utiliser uniquement si prescrit (risque d'absorption percutanée)

> [!warning] Ne jamais mélanger avec la Bétadine
> La chlorhexidine et la povidone-iodée (Bétadine) sont **chimiquement incompatibles** → formation d'un complexe inactif + potentiellement irritant. Si on change d'antiseptique entre deux soins, laisser sécher complètement le premier.

---

## Ce que je fais en tant qu'IDE

- [ ] Demander les allergies (chlorhexidine, latex, nickel) avant tout soin
- [ ] Application en cercles concentriques, du centre vers l'extérieur (ne pas repasser sur une zone déjà désinfectée)
- [ ] Respecter le temps de séchage (au moins 30 secondes) avant le geste → si on n'attend pas, ça ne sert à rien
- [ ] Dialyse péritonéale : rincer le connecteur à la chlorhexidine à chaque échange (protocole service)
- [ ] Conserver les flacons à l'abri de la lumière et vérifier la date de péremption (et la date d'ouverture, souvent 30 jours)

---

## Les 3 choses à retenir

1. **Jamais avec la Bétadine** (incompatibilité chimique, l'une neutralise l'autre)
2. **Laisser sécher** avant le geste → si on pique dans le mouillé, l'antisepsie est nulle
3. **Allergie possible** → toujours interroger, même pour un soin "banal"

## Voir aussi

[[Soins Infirmiers/Cathéter intra péritonéal\|Dialyse péritonéale]] · [[Soins Infirmiers/Pose VVP\|VVP]] · [[Cathéter veineux central\|Cathéter central]]

> [!caution] Outil pédagogique → Toujours vérifier les protocoles du service

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
