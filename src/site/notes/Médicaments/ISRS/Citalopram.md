---
{"dg-publish":true,"permalink":"/Médicaments/ISRS/Citalopram/","tags":["médicament","ISRS","antidépresseur","psychiatrie","QT"],"noteIcon":"","dg-note-properties":{"nom":"Citalopram","classe":["ISRS","Antidépresseur"],"indications":["Dépression caractérisée","Troubles paniques","Troubles anxieux généralisés (TAG)"],"posologie":"20 mg/j en 1 prise. Max 40 mg/j adulte. Max 20 mg/j si > 65 ans, insuffisance hépatique ou faible métaboliseur CYP2C19","contre_indications":["IMAO (délai 14 jours)","Pimozide (QT → CI absolue)","Syndrome du QT long congénital","Allongement QT préexistant (> 450 ms homme, > 470 ms femme)"],"effets_secondaires":["Allongement QTc dose-dépendant (alerte ANSM 2011 → plus marqué que sertraline/fluoxétine)","Nausées, insomnies","Dysfonctions sexuelles","Syndrome de sevrage (arrêt brutal)","Hyponatrémie (SIADH → sujet âgé)"],"service":["Psychiatrie","Médecine générale"],"mécanisme":"ISRS mélange racémique (escitalopram = énantiomère S actif seul). Même mécanisme d'inhibition SERT. Alerte ANSM 2011 : allongement QTc dose-dépendant → limites de dose strictes. L'escitalopram est souvent préféré.","interactions":["IMAO → CI absolue","Médicaments allongeant le QT → CI/précaution","Tramadol, triptans → syndrome sérotoninergique"],"niveau_preuve":"A","tags":["médicament","ISRS","antidépresseur","psychiatrie","QT"],"revision":"2026-06-13"}}
---


> [!abstract] En 2 mots
> ISRS avec risque d'allongement du QT dose-dépendant (alerte ANSM 2011). Doses strictement limitées. L'escitalopram (énantiomère actif) est souvent préféré aujourd'hui.

---

## À quoi ça sert ?

- **Dépression caractérisée**
- **Troubles paniques**
- **Troubles anxieux généralisés** (TAG)

---

## Comment ça marche ? (simplifié)

Même mécanisme que la sertraline : blocage de la pompe de recapture de la sérotonine (SERT). Le citalopram est un mélange de deux molécules miroir (un racémate) : seul l'énantiomère S (escitalopram) est actif.

> Analogie : le citalopram, c'est comme une paire de gants gauche et droit → seul le gauche (l'escitalopram) s'encastre correctement dans le récepteur. Le gant droit est là mais ne fait rien d'utile.

**Problème important** : en 2011, l'ANSM a alerté sur un allongement du QT dose-dépendant → risque de troubles du rythme cardiaque graves → limites posologiques strictes.

---

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - Syndrome du QT long congénital
> - Association avec le pimozide
> - Ne pas dépasser 40 mg/j (20 mg/j chez > 65 ans et insuffisant hépatique)

> [!warning] Précautions importantes
> - ECG recommandé si facteurs de risque d'allongement QT (hypokaliémie, autres médicaments QT)
> - Mêmes CI IMAO que tous les ISRS
> - Arrêt progressif obligatoire

> [!caution] À surveiller
> - Natrémie chez le sujet âgé
> - Signes de syndrome de sevrage si oubli de dose

---

## Ce que je fais en tant qu'IDE

- [ ] Vérifier que la dose ne dépasse pas 40 mg/j (20 mg/j chez > 65 ans)
- [ ] ECG si facteurs de risque QT ou association avec d'autres médicaments QT
- [ ] Mêmes surveillances ISRS : délai 2-4 semaines, idées suicidaires, natrémie
- [ ] Informer sur le sevrage progressif

---

## Les 3 choses à retenir absolument

1. **Allongement QT dose-dépendant** (alerte ANSM 2011) → dose strictement ≤ 40 mg/j (≤ 20 mg/j chez > 65 ans)
2. **L'escitalopram est souvent préféré** (énantiomère actif seul, profil légèrement meilleur)
3. **CI avec pimozide** et allongement QT préexistant

---

## Voir aussi

[[Médicaments/ISRS/Escitalopram\|Escitalopram (énantiomère actif, préféré)]] · [[Médicaments/ISRS/Sertraline\|Sertraline (mécanisme ISRS complet)]]

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service
