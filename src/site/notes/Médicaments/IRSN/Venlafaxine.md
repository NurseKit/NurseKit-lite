---
{"dg-publish":true,"permalink":"/Médicaments/IRSN/Venlafaxine/","tags":["médicament","IRSN","antidépresseur","anxiété","psychiatrie"],"noteIcon":"","dg-note-properties":{"nom":"Venlafaxine","classe":["IRSN","Antidépresseur","Inhibiteur de la Recapture de la Sérotonine et de la Noradrénaline"],"indications":["Dépression caractérisée","Troubles anxieux généralisés (TAG)","Troubles paniques","Phobie sociale","Douleurs neuropathiques (hors AMM → usage courant)"],"posologie":"Forme LP : initiation 37,5 mg/j pendant 7j → 75 mg/j. Cible 75-225 mg/j en 1 prise. Max 375 mg/j (formes résistantes en hospitalier). Ne jamais écraser les comprimés LP","contre_indications":["IMAO (délai 14 jours)","HTA non contrôlée (effet hypertenseur dose-dépendant)","Infarctus du myocarde récent","Glaucome à angle fermé"],"effets_secondaires":["Nausées (fréquentes → atténuées par forme LP et prise au repas)","HTA dose-dépendante (surtout > 150 mg/j) → surveiller PA","Tachycardie","Sueurs, sécheresse buccale","Insomnie, agitation","Dysfonctions sexuelles","Syndrome de sevrage très sévère (parmi les pires des antidépresseurs)"],"service":["Psychiatrie","Médecine interne","Médecine générale"],"mécanisme":"IRSN double action dose-dépendante : ≤ 75 mg/j = inhibition SERT surtout (profil ISRS-like). > 150 mg/j = inhibition SERT + NET (noradrénaline). Double action → efficacité accrue dans dépression résistante, douleurs neuropathiques. L'activation noradrénergique = ↑ PA et FC.","interactions":["IMAO → syndrome sérotoninergique (CI absolue)","Tramadol, triptans, millepertuis, lithium → syndrome sérotoninergique","AINS, anticoagulants → ↑ saignements","Antihypertenseurs → antagonisme partiel (↑ PA)"],"niveau_preuve":"A","tags":["médicament","IRSN","antidépresseur","anxiété","psychiatrie"],"revision":"2026-06-13"}}
---


> [!abstract] En 2 mots
> Antidépresseur à double action dose-dépendante : à faible dose il se comporte comme un ISRS, à forte dose il ajoute la noradrénaline. Efficace dans les dépressions résistantes. Deux dangers à ne jamais oublier : la pression artérielle qui monte avec la dose, et le syndrome de sevrage parmi les plus violents de toute la pharmacologie psychiatrique.

---

## À quoi ça sert ?

- **Dépression caractérisée** (1ère ligne ou après échec d'un ISRS)
- **Troubles anxieux généralisés** (TAG)
- **Troubles paniques**, **phobie sociale**
- Douleurs neuropathiques (hors AMM, très utilisé en pratique clinique)

---

## Comment ça marche ? (simplifié)

La venlafaxine ajuste son profil selon la dose :
- **Faible dose (≤ 75 mg/j)** : surtout sérotoninergique → profil proche des ISRS
- **Dose plus élevée (> 150 mg/j)** : noradrénaline s'ajoute → double action sérotonine + noradrénaline

La noradrénaline contribue à l'énergie, à la concentration, et active les voies qui "éteignent" la douleur → d'où son intérêt dans les douleurs neuropathiques.

> Analogie : à faible dose, c'est comme renforcer une rivière (sérotonine). À dose élevée, un second cours d'eau rejoint la première (noradrénaline) → plus de débit, plus d'efficacité dans les dépressions résistantes. Mais ce second cours d'eau fait aussi monter la pression.

Délai d'efficacité : **2 à 4 semaines** (comme tous les antidépresseurs, ne pas abandonner trop tôt).

---

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - **IMAO** (délai 14 jours) → syndrome sérotoninergique mortel → urgence vitale
> - **Ne jamais arrêter brutalement** : le syndrome de sevrage de la venlafaxine est parmi les plus intenses de tous les antidépresseurs (vertiges rotatoires, nausées, paresthésies "chocs électriques", irritabilité, confusion)

> [!warning] Précautions importantes
> - **HTA non contrôlée** : contre-indiquée → la venlafaxine augmente la PA de façon dose-dépendante, surtout au-delà de 150 mg/j
> - Mesurer la PA avant ET régulièrement pendant le traitement
> - Ne pas écraser les comprimés LP (libération prolongée, si écrasés, libération de toute la dose d'un coup)

> [!caution] À surveiller
> - PA et FC (surtout à partir de 150 mg/j)
> - Natrémie chez le sujet âgé (SIADH possible)
> - Idées suicidaires en début de traitement chez les moins de 25 ans

---

## Ce que je fais en tant qu'IDE

- [ ] Mesurer **PA et FC** avant initiation et à chaque consultation (surtout > 150 mg/j)
- [ ] Prise avec le repas (↓ nausées) → forme LP uniquement, ne pas écraser ni croquer
- [ ] Informer sur le délai d'efficacité (2 à 4 semaines) → ne pas s'attendre à un effet immédiat
- [ ] **Jamais d'arrêt brutal** : sevrage sur minimum 4 semaines par paliers de 37,5 mg → informer le patient
- [ ] Prévenir que même un simple oubli peut déclencher des symptômes de sevrage (chocs électriques, vertiges)
- [ ] Surveiller la natrémie chez les patients âgés

---

## Les 3 choses à retenir absolument

1. **Surveiller la PA** → effet hypertenseur dose-dépendant → CI si HTA non contrôlée, ↑ à surveiller surtout > 150 mg/j
2. **Syndrome de sevrage parmi les plus sévères** des antidépresseurs → arrêt très progressif sur plusieurs semaines
3. **Double action dose-dépendante** : sérotonine seule à faible dose, noradrénaline en renfort au-delà de 150 mg/j

---

## Voir aussi

[[Médicaments/ISRS/Sertraline\|Sertraline (ISRS, 1ère ligne dépression)]] · [[Médicaments/IRSN/Duloxetine\|Duloxétine (IRSN, douleur neuropathique)]] · [[Pathologies/Dépression\|Dépression caractérisée]]

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service
