---
{"dg-publish":true,"permalink":"/Médicaments/Bêta-bloquants/Metoprolol/","tags":["médicament","bêta-bloquant","cardiosélectif","cardiologie","HTA","IC"],"noteIcon":"","dg-note-properties":{"nom":"Métoprolol","classe":["Bêta-bloquant","Bêta-1 sélectif (cardiosélectif)"],"indications":["Hypertension artérielle","Angor stable","Insuffisance cardiaque stable (titration lente)","Infarctus du myocarde (prévention secondaire)","Tachycardie sinusale","FA → contrôle de fréquence","Cardiomyopathie hypertrophique obstructive"],"posologie":"HTA/angor : 50-200 mg/j en 1-2 prises (forme LP). IC : titration 12,5 mg/j → 200 mg/j (ZOK LP). IV urgence : 5 mg en 5 min répété x3","contre_indications":["Bradycardie < 50/min","BAV 2 ou 3 non appareillé","Hypotension sévère","Asthme sévère (CI relative)","Choc cardiogénique"],"effets_secondaires":["Bradycardie","Hypotension orthostatique","Fatigue, asthénie","Extrémités froides","Bronchospasme (relatif)","Masquage hypoglycémie"],"service":["Cardiologie","Médecine interne","Urgences"],"mécanisme":"Bêta-bloquant cardiosélectif β1 (sélectivité β1/β2 ≈ 70:1) → ↓ FC, contractilité, conduction AV, automaticité → ↓ consommation O₂ myocardique. Remodelage ventriculaire inverse dans l'IC à long terme.","interactions":["Vérapamil/diltiazem IV → BAV/bradycardie sévère (CI IV)","Amiodarone → bradycardie synergique","Digoxine → bradycardie additive","Clonidine → rebond hypertensif à l'arrêt"],"niveau_preuve":"A","tags":["médicament","bêta-bloquant","cardiosélectif","cardiologie","HTA","IC"],"revision":"2026-06-13"}}
---


> [!abstract] En 2 mots
> Bêta-bloquant cardiosélectif β1 utilisé dans l'HTA, l'angor, l'IC et les arythmies. Principal risque IDE : ne JAMAIS arrêter brutalement (rebond) → vérifier FC avant chaque prise.

## 💊 À quoi ça sert ?

- Hypertension artérielle
- Angor stable
- Insuffisance cardiaque avec FEVG réduite (titration lente, étude MERIT-HF)
- Prévention secondaire post-IDM
- Tachycardie sinusale et contrôle de fréquence dans la FA
- IV en urgence pour tachyarythmies supraventriculaires

## 🔬 Comment ça marche ? (simplifié)

Le métoprolol bloque les récepteurs bêta-1 du cœur, ce qui ralentit le rythme cardiaque, réduit la force de contraction et diminue la tension artérielle. Sa cardiosélectivité (β1 >> β2) le rend moins dangereux pour les bronches que le propranolol, sans toutefois être sans risque à fortes doses. Dans l'IC, malgré l'effet dépresseur à court terme, le blocage de la stimulation sympathique chronique améliore le pronostic à long terme.
> 💡 Analogie : le métoprolol met le cœur en mode "économie d'énergie" ciblé → il ralentit le cœur sans trop toucher aux poumons (contrairement au propranolol).

## ⚠️ Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - Bradycardie < 50/min
> - BAV de 2e ou 3e degré non appareillé
> - Choc cardiogénique
> - Hypotension sévère (PAS < 90 mmHg)
> - NE JAMAIS ARRÊTER BRUTALEMENT (rebond = crise hypertensive, angor instable, IDM)

> [!warning] Interactions majeures
> - Vérapamil ou diltiazem IV : BAV complet et arrêt cardiaque possible (CI absolue en IV)
> - Amiodarone : bradycardie et BAV synergiques

> [!caution] À surveiller
> - FC avant chaque administration : ne pas donner si FC < 50/min
> - TA (hypotension orthostatique)
> - Glycémie chez le diabétique (masque la tachycardie hypoglycémique)
> - Signes d'aggravation de l'IC à l'initiation

## 🩺 Ce que je fais en tant qu'IDE

- [ ] Mesurer FC avant chaque prise → ne pas administrer si FC < 50/min
- [ ] Mesurer la TA (hypotension orthostatique)
- [ ] Ne JAMAIS arrêter brutalement : sevrage progressif sur ≥ 2 semaines
- [ ] Chez le diabétique : renforcer l'autosurveillance glycémique (masque la tachycardie)
- [ ] À l'initiation dans l'IC : surveiller les signes de rétention hydrique (poids, œdèmes)

## 📌 Les 3 choses à retenir absolument

1. Ne JAMAIS arrêter brutalement → sevrage progressif sur ≥ 2 semaines (risque de rebond)
2. FC < 50/min avant la prise = ne pas administrer et alerter
3. Cardiosélectif β1 mais attention aux fortes doses chez l'asthmatique (sélectivité relative)

## 🔗 Voir aussi

- [[Médicaments/Bêta-bloquants/Atenolol\|Aténolol (BB cardiosélectif hydrosoluble)]]
- [[Médicaments/Bêta-bloquants/Carvedilol\|Carvédilol (BB non sélectif + α-bloquant, IC)]]
- [[Médicaments/Bêta-bloquants/Propranolol\|Propranolol (BB non sélectif liposoluble)]]

> [!caution] Outil pédagogique
