---
{"dg-publish":true,"permalink":"/medicaments/beta-bloquants/propranolol/","tags":["médicament","hospitalier","beta-bloquant"],"noteIcon":"","dg-note-properties":{"nom":"Propranolol","classe":["Bêta-bloquant"],"indications":["HTA","Angor stable","Troubles du rythme (tachycardie supraventriculaire, FA)","Tremblement essentiel","Prévention de la migraine","Anxiété de performance","Hyperthyroïdie (symptômes)","Hémangiomes infantiles","Prévention des varices oesophagiennes (hypertension portale)"],"posologie":"HTA/Angor : 40-160 mg/j en 2-3 prises. Tremblement/Anxiété : 10-40 mg avant l'événement. Migraine préventive : 40-80 mg x2/j","contre_indications":["Asthme bronchique (absolu → non cardiosélectif)","BPCO obstructive (relatif mais prudence extrême)","BAV de 2e/3e degré non appareillé","Bradycardie <50/min","Choc cardiogénique","IC décompensée","Syndrome du sinus malade","Phéochromocytome non traité (risque de crise HTA paradoxale)"],"effets_secondaires":["Bradycardie","Bronchospasme (risque élevé → non sélectif β2)","Hypotension","Fatigue","Extrémités froides","Cauchemars, troubles du sommeil (liposoluble → passage BHE++)","Masque signes hypoglycémie","Syndrome de Raynaud aggravé","Dysfonction érectile"],"service":["Cardiologie","Neurologie","Dermatologie pédiatrique","Hépato-gastroentérologie"],"mécanisme":"Bêta-bloquant non cardiosélectif (β1+β2). Liposoluble → passe BHE. Effets: chronotrope/inotrope/dromotrope/bathmotrope négatifs, ↓ rénine, bronchospasme par blocage β2. Classe II Vaughan-Williams.","interactions":["Vérapamil/diltiazem : BAV et arrêt cardiaque","Amiodarone : bradycardie et BAV","Adrénaline : HTA paradoxale (α non antagonisé)","AINS : ↓ effet antihypertenseur","Antidiabétiques : masque hypoglycémie"],"niveau_preuve":"A","tags":["médicament","hospitalier","beta-bloquant"],"revision":"2026-06-22"}}
---

> [!abstract] En 2 mots
> Bêta-bloquant non sélectif liposoluble, prototype de la classe, avec des indications larges (HTA, arythmies, migraine, tremblement, anxiété). Principal risque IDE : bronchospasme (CONTRE-INDIQUÉ dans l'asthme) et ne jamais arrêter brutalement.

## À quoi ça sert ?

- Hypertension artérielle, angor stable
- Arythmies (FA, tachycardies supraventriculaires)
- Tremblement essentiel (1ère intention)
- Prévention de la migraine
- Anxiété de performance (prise unique avant événement)
- Contrôle des symptômes cardiovasculaires dans l'hyperthyroïdie
- Hémangiomes infantiles (Hemangiol®)
- Prévention hémorragie variqueuse (hypertension portale)

## Comment ça marche ? (simplifié)

Le propranolol bloque les récepteurs bêta-1 (cœur) ET bêta-2 (bronches, vaisseaux). Il ralentit le cœur, réduit la tension artérielle et, grâce à sa liposolubilité, passe dans le cerveau pour réduire les tremblements et l'anxiété. Mais le blocage bêta-2 bronchique provoque un bronchospasme : c'est pourquoi il est absolument contre-indiqué dans l'asthme.
> Analogie : le propranolol met le cœur ET les poumons en mode "moins réactifs" → utile pour le cœur et le cerveau, mais les poumons ne peuvent plus se dilater correctement chez l'asthmatique.

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - Asthme bronchique (CI ABSOLUE, blocage β2 = bronchospasme potentiellement fatal)
> - BAV de 2e ou 3e degré non appareillé
> - Bradycardie < 50/min
> - Choc cardiogénique, IC décompensée
> - Phéochromocytome non traité (HTA paradoxale)
> - NE JAMAIS ARRÊTER BRUTALEMENT (rebond)

> [!warning] Interactions majeures
> - Vérapamil ou diltiazem IV : arrêt cardiaque possible
> - Amiodarone : bradycardie et BAV sévères
> - Adrénaline : HTA paradoxale (les β2 vasodilatateurs sont bloqués, seul l'effet α vasoconstricteur reste)

> [!caution] À surveiller
> - FC avant chaque administration : ne pas donner si FC < 50/min
> - Signes de bronchospasme (dyspnée sifflante = urgence)
> - Glycémie chez le diabétique (masque tachycardie ET tremblements hypoglycémiques)
> - Troubles neuropsychiques (cauchemars, insomnie, dépression, liposoluble)

## Ce que je fais en tant qu'IDE

- [ ] Interroger SYSTÉMATIQUEMENT sur les antécédents d'asthme avant administration
- [ ] Mesurer FC avant chaque prise → ne pas administrer si FC < 50/min
- [ ] Surveiller les signes de bronchospasme (dyspnée sifflante = urgence médicale)
- [ ] Ne JAMAIS arrêter brutalement : sevrage progressif sur ≥ 2 semaines
- [ ] Chez le diabétique : masque tachycardie ET tremblements (plus complet que les cardiosélectifs) → renforcer l'autosurveillance glycémique
- [ ] Informer des troubles du sommeil possibles (cauchemars, insomnie, liposolubilité élevée)

## Les 3 choses à retenir

1. Non sélectif (β1+β2) = ASTHME CONTRE-INDIQUÉ ABSOLUMENT (bronchospasme fatal possible)
2. Liposoluble = passe dans le cerveau = effets neuropsychiques fréquents (cauchemars, dépression) ET utile pour tremblements/anxiété
3. Ne JAMAIS arrêter brutalement → et chez le diabétique : masque PLUS de signes d'hypoglycémie (tachycardie + tremblements bloqués)

## Voir aussi

- [[Médicaments/Bêta-bloquants/Atenolol\|Aténolol (BB cardiosélectif hydrosoluble, moins d'effets SNC)]]
- [[Médicaments/Bêta-bloquants/Metoprolol\|Métoprolol (BB cardiosélectif)]]
- [[Médicaments/Bêta-bloquants/Carvedilol\|Carvédilol (BB non sélectif + α-bloquant, IC)]]
- [[index\|Tous les bêta-bloquants]]

> [!caution] Outil pédagogique

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
