---
{"dg-publish":true,"permalink":"/medicaments/vitamines/vitamine-k/","tags":["médicament","vitamine","vitamine_k","antidote_avk","warfarine","INR","PPSB"],"noteIcon":"","dg-note-properties":{"nom":"Vitamine K","classe":["Vitamine liposoluble","Co-facteur de carboxylation"],"indications":["Surdosage AVK (warfarine, acénocoumarol) → antidote","Maladie hémorragique du nouveau-né (prévention)","Carence en vitamine K"],"posologie":"Surdosage AVK asymptomatique : vit K1 PO 1-5 mg · Hémorragie grave : vit K1 IV 5-10 mg + PPSB","contre_indications":["Injection IV trop rapide (anaphylaxie rare)"],"effets_secondaires":["Résistance aux AVK si surdosage (effet rebond 1-2 semaines)","Anaphylaxie (IV rapide → très rare)"],"service":["Urgences","Réanimation","Hématologie","Maternité"],"niveau_preuve":"A","tags":["médicament","vitamine","vitamine_k","antidote_avk","warfarine","INR","PPSB"],"revision":"2026-06-22"}}
---


> [!abstract] En 2 mots
> La vitamine K1 (Konakion®) est l'antidote des AVK (warfarine, acénocoumarol). Efficacité différée (4-6h PO, 2h IV) → si hémorragie grave : associer le PPSB pour une correction immédiate de l'INR. Sous AVK : ne pas modifier brutalement la consommation alimentaire en vitamine K (régularité plutôt que suppression).

## À quoi ça sert ?

- Antidote des AVK (surdosage ou hémorragie)
- Prévention de la maladie hémorragique du nouveau-né (K1 à la naissance)
- Traitement des carences en vitamine K (malabsorption, antibiotiques prolongés)

## Comment ça marche ? (simplifié)

Les AVK bloquent la régénération de la vitamine K → les facteurs de coagulation (II, VII, IX, X) ne sont plus activés → l'INR monte. La vitamine K1 (Konakion®) restaure la régénération des facteurs → l'INR revient à la normale en 4-6h (PO) ou 2h (IV). Pour une correction immédiate (urgence hémorragique), il faut aussi injecter du PPSB (facteurs préformés).

## Ce qu'il ne faut PAS faire

> [!danger] CI
> - Injection IV trop rapide → risque d'anaphylaxie (perfuser en 15-30 min)
> - Dose trop élevée de vitamine K → résistance prolongée aux AVK (1-2 semaines) → difficile de ré-anticoaguler si nécessaire

> [!warning] Précautions
> - Sous AVK : les variations alimentaires de K (épinards, choux) perturbent l'INR → conseiller **régularité** de la consommation, pas suppression

> [!caution] Surveiller
> - INR à 6h après traitement IV (ou 24h après PO)
> - Hémorragie grave : associer PPSB (correction immédiate) + vit K1 (consolidation)

## Ce que je fais en tant qu'IDE

- [ ] Vitamine K IV : **diluer dans SG5%**, perfuser en **15-30 min** (anaphylaxie si trop rapide)
- [ ] Contrôle INR à 6h après IV ou 24h après PO
- [ ] Alimentation sous AVK : **régularité** de consommation en légumes verts (pas de suppression)

### Algorithme surdosage AVK

| Situation | Conduite |
|-----------|---------|
| INR > 4, sans hémorragie | Passer une dose · Si > 8 : vit K1 PO 1-5 mg · Contrôle à 24h |
| Hémorragie peu sévère + INR > 3 | Vit K1 PO 1-5 mg |
| **Hémorragie grave** (quel que soit l'INR) | **PPSB 25 UI/kg IV** + **Vit K1 10 mg IV lente (15 min)** → urgences/réa |

## Les 3 choses à retenir

> [!caution] Outil pédagogique
> 1. Antidote AVK = **Konakion®** (K1) → mais efficacité **différée** → associer PPSB si urgence hémorragique
> 2. Vit K IV : toujours **en perfusion lente 15-30 min** (jamais en bolus, anaphylaxie)
> 3. Sous AVK : ne **pas supprimer** la vitamine K alimentaire → maintenir une consommation **régulière**

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
