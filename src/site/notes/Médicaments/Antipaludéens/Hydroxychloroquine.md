---
{"dg-publish":true,"permalink":"/Médicaments/Antipaludéens/Hydroxychloroquine/","tags":["médicament","antipaludéen","immunomodulateur","lupus","rhumatologie","rétinopathie_surveillance"],"noteIcon":"","dg-note-properties":{"nom":"Hydroxychloroquine","classe":["Antipaludéen","Immunomodulateur","Antihumatismal de fond (DMARDs)"],"indications":["Lupus érythémateux systémique (LES) → traitement de fond indispensable","Polyarthrite rhumatoïde (2ème ligne ou en association)","Syndrome de Gougerot-Sjögren","Lucite polymorphe (réaction cutanée solaire)","Paludisme zones sensibles (de moins en moins → résistances)"],"posologie":"LES/PR : 200-400 mg/j PO (max 5 mg/kg/j de poids réel, ne jamais dépasser). Prise unique ou en 2 prises avec repas","contre_indications":["Rétinopathie maculaire préexistante","Association tamoxifène (↑↑ risque rétinopathie → CI relative)","Antécédent de myopathie liée aux antipaludéens de synthèse","Insuffisance rénale ou hépatique sévère"],"effets_secondaires":["Rétinopathie (dose-cumulée et durée-dépendante → principal risque long terme, irréversible)","Troubles GI (nausées, diarrhées → s'atténuent avec repas)","Rash cutané, prurit","Allongement QT (moins fréquent que chloroquine)","Hypoglycémie possible (effet insulino-sensibilisant)"],"service":["Rhumatologie","Médecine interne","Dermatologie"],"mécanisme":"Accumulation lysosomale → ↑ pH vacuolaire → modulation de la présentation antigénique → ↓ activation lymphocytes T et B → effet anti-inflammatoire et immunomodulateur. Effet antiparasitaire par inhibition polymérisation hème (même mécanisme que chloroquine)","interactions":["Tamoxifène → ↑↑ risque rétinopathie (CI relative → surveiller)","Antidiabétiques → ↑ hypoglycémie (insulino-sensibilisant)","Médicaments allongeant QT (amiodarone, azithromycine) → risque additif","Antiacides → ↓ absorption (espacer 4h)","Digoxine → ↑ digoxinémie"],"niveau_preuve":"A (LES, PR)","tags":["médicament","antipaludéen","immunomodulateur","lupus","rhumatologie","rétinopathie_surveillance"],"revision":"2026-06-13"}}
---


> [!abstract] En 2 mots
> Pilier indispensable du traitement du lupus → sans elle, le risque de poussée augmente. Son usage principal n'est plus antipaludéen mais rhumatologique. Le risque à ne jamais perdre de vue : la rétinopathie irréversible qui nécessite un fond d'œil annuel dès 5 ans de traitement.

---

## À quoi ça sert ?

Tu verras le Plaquenil® en rhumatologie bien plus souvent qu'en médecine des voyages :
- **Lupus érythémateux systémique (LES)** : quasi-systématique dans la prise en charge → réduit les poussées, protège les reins et le cœur, améliore la survie sur le long terme, réduit le risque de thrombose
- **Polyarthrite rhumatoïde** : souvent associée au méthotrexate pour renforcer l'effet de fond
- Syndrome de Gougerot-Sjögren (sécheresse oculaire et buccale auto-immune)
- Lucite polymorphe (réaction cutanée au soleil)

---

## Comment ça marche ? (simplifié)

L'hydroxychloroquine s'accumule à l'intérieur des cellules du système immunitaire et modifie leur environnement interne (elle remonte le pH des lysosomes). Cela perturbe la façon dont les cellules présentent les antigènes → moins de réactions auto-immunes mal dirigées → moins de poussées de lupus ou d'arthrite.

> Dans le lupus, le système immunitaire confond le "soi" avec un ennemi et déclenche des fausses alertes inflammatoires. L'hydroxychloroquine filtre et atténue ces fausses alertes, sans bloquer l'immunité dans son ensemble.

Bonus : légère protection cardiovasculaire (anti-agrégant) et effet insulino-sensibilisant → particulièrement bénéfiques dans le lupus où les complications cardiaques et métaboliques sont fréquentes.

---

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - **Rétinopathie maculaire préexistante** : risque d'aggravation irréversible
> - Ne jamais dépasser **5 mg/kg/j de poids réel** → utiliser le vrai poids du patient, pas le poids idéal, c'est important pour le calcul

> [!warning] Précautions
> - **Tamoxifène** (traitement du cancer du sein) : association qui multiplie le risque de rétinopathie → surveillance ophtalmologique renforcée
> - Insuffisance rénale ou hépatique sévère : accumulation → réduire la dose
> - Médicaments allongeant le QT : risque additif (ECG si cardiopathie)
> - Grossesse : peut être maintenu dans le LES → la grossesse lupique non traitée comporte plus de risques que le médicament

> [!caution] À surveiller
> - **Rétinopathie** : fond d'œil + champ visuel + OCT maculaire avant traitement, puis **annuellement dès 5 ans** (ou plus tôt si dose élevée, IR sévère, ou association tamoxifène)
> - **Glycémie** si patient diabétique : effet insulino-sensibilisant → risque d'hypoglycémie
> - Troubles visuels (vision floue, halos, modification de la vision des couleurs) → ophtalmo urgent

---

## Ce que je fais en tant qu'IDE

- [ ] Vérifier qu'un **bilan ophtalmologique** (fond d'œil + champ visuel) a été réalisé avant le début du traitement
- [ ] Rappeler la surveillance **annuelle dès 5 ans** → c'est souvent oublié par les patients sur le long terme
- [ ] Calculer et vérifier que la dose ne dépasse pas **5 mg/kg/j de poids réel** → recalculer si le poids change
- [ ] Prendre avec les repas (réduit les nausées initiales)
- [ ] Éduquer sur la **protection solaire** (photosensibilité possible)
- [ ] Surveiller la glycémie si patient diabétique (effet hypoglycémiant possible)
- [ ] Signaler tout trouble visuel → consultation ophtalmologique urgente
- [ ] **Ne jamais arrêter** sans avis rhumatologique : le risque de poussée lupique à l'arrêt est réel

---

## Les 3 choses à retenir absolument

1. **Traitement de fond du lupus → ne jamais arrêter sans avis médical** : l'arrêt du Plaquenil® peut déclencher une poussée lupique sévère
2. **Fond d'œil annuel dès 5 ans** de traitement → la rétinopathie est irréversible si non dépistée à temps
3. **Dose max = 5 mg/kg/j de POIDS RÉEL** → ne pas utiliser le poids idéal, calculer avec le vrai poids

---

## Voir aussi

- [[Médicaments/Antipaludéens/Chloroquine\|Chloroquine]] (même famille, rétinopathie plus fréquente)
- [[Médicaments/Immunosuppresseurs/Methotrexate\|Méthotrexate]] (souvent associé dans la PR)
- [[Médicaments/Antipaludéens/Atovaquone Proguanil\|Malarone®]] · [[Médicaments/Antipaludéens/Mefloquine\|Méfloquine]]

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service
