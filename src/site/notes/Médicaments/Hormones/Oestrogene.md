---
{"dg-publish":true,"permalink":"/medicaments/hormones/oestrogene/","tags":["médicament","hormone","œstrogène","THS","contraception","thrombose"],"noteIcon":"","dg-note-properties":{"nom":"Oestrogène","classe":["Hormone sexuelle féminine","Estrogène naturel / synthétique"],"indications":["THS ménopause (symptômes vasomoteurs)","Contraception hormonale (œstro-progestative)","Hypogonadisme féminin","Prévention ostéoporose post-ménopausique","Cancer de la prostate (hors AMM)"],"posologie":"THS : gel 0,5-1,5 mg estradiol/j (Estreva®, Oestrogel®), patch 25-50 µg/j · Contraception : EE 20-35 µg/cp (selon pilule) · Toujours associer un progestatif si utérus intact","contre_indications":["Antécédent de cancer du sein (ou suspicion)","Antécédent de MTEV sans anticoagulation adéquate","Maladie thrombo-embolique active","Cardiopathie coronarienne, AVC","Lupus","Porphyrie"],"effets_secondaires":["Risque thromboembolique veineux (↑ facteurs coagulation → surtout voie orale)","Cancer du sein (↑ avec durée > 5 ans)","Cancer de l'endomètre si progestatif absent","Tensions mammaires, nausées","Rétention hydrosodée","Migraines"],"service":["Gynécologie","Endocrinologie"],"mécanisme":"Liaison aux récepteurs ERα et ERβ nucléaires. Voie orale → 1er passage hépatique → ↑ facteurs de coagulation → risque thrombotique. Voie transdermique → évite cet effet → risque thrombotique moindre.","interactions":["Rifampicine, carbamazépine, phénytoïne (inducteurs CYP) : ↓ efficacité (contraception inefficace)","Anticoagulants : antagonisation possible"],"niveau_preuve":"A","tags":["médicament","hormone","œstrogène","THS","contraception","thrombose"],"revision":"2026-06-22"}}
---

> [!abstract] En 2 mots
> L'hormone féminine de remplacement → indispensable dans le THS de la ménopause et la contraception. À retenir absolument : la voie orale passe par le foie et augmente le risque de phlébite, la voie transdermique l'évite. Et si l'utérus est présent, le progestatif n'est jamais optionnel.

## À quoi ça sert ?

- Traiter les symptômes de la ménopause (bouffées de chaleur, sécheresse vaginale)
- Prévenir l'ostéoporose post-ménopausique
- Contraception (toujours en association avec un progestatif, pilule combinée)
- Hypogonadisme féminin (ovaires qui ne fonctionnent plus)

## Comment ça marche ? (simplifié)

Les œstrogènes remplacent l'estradiol que les ovaires ont arrêté de produire. Le point clé à comprendre, c'est la voie d'administration. En passant par la bouche, l'hormone traverse le foie en premier → et le foie réagit en fabriquant plus de facteurs de coagulation, ce qui augmente le risque de phlébite. En patch ou en gel, elle entre directement dans le sang, sans ce péage hépatique.

> Retiens l'image : voie orale = passage obligatoire par le péage hépatique (avec taxe thrombotique). Voie transdermique = autoroute directe sans péage. Chez les femmes à risque cardiovasculaire, la transdermique s'impose.

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - ATCD de cancer du sein ou suspicion → contre-indication formelle
> - MTEV active ou ATCD de phlébite/embolie → risque majoré
> - Cardiopathie coronarienne, AVC
> - Tabac > 35 ans pour la contraception orale combinée

> [!warning] Interactions majeures
> - Inducteurs enzymatiques (rifampicine, carbamazépine, phénytoïne) → ↓ efficacité contraceptive → risque de grossesse non désirée
> - Anticoagulants → interactions complexes à surveiller

> [!caution] À surveiller
> - Préférer la voie transdermique chez les femmes à risque cardiovasculaire ou thrombotique
> - **Toujours associer un progestatif** si l'utérus est intact → sans lui, on expose l'endomètre à un risque de cancer
> - Surveiller PA, tensions mammaires, poids à chaque consultation
> - Réévaluer le rapport bénéfice/risque au moins une fois par an

## Ce que je fais en tant qu'IDE

- [ ] Appliquer le gel ou changer le patch selon protocole (rotation des sites)
- [ ] Éduquer : ne jamais appliquer le gel sur les seins ou le visage
- [ ] Gel transdermique : rotation sur avant-bras, abdomen, cuisses → ne pas frotter
- [ ] Surveiller signes de thrombose : douleur mollet, dyspnée, douleur thoracique → alerter immédiatement
- [ ] Vérifier que le progestatif est bien associé si utérus présent
- [ ] Éduquer sur les CI (tabac surtout si contraception orale, antécédents cardio)

## Les 3 choses à retenir

1. **Voie transdermique préférable** en cas de risque cardiovasculaire → elle court-circuite l'effet thrombogène hépatique
2. **Toujours associer un progestatif** si l'utérus est présent → protection de l'endomètre obligatoire
3. CI absolues : ATCD cancer sein, MTEV, tabac > 35 ans pour la contraception orale combinée

## Voir aussi

[[Médicaments/Hormones/Testosterone\|Testostérone]] · [[Médicaments/Hormones/Levothyroxine\|Lévothyroxine]] · TP-INR · Contraceptifs

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
