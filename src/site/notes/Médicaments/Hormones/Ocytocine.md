---
{"dg-publish":true,"permalink":"/Médicaments/Hormones/Ocytocine/","tags":["médicament","hormone","maternité","utérotonique","obstétrique"],"noteIcon":"","dg-note-properties":{"nom":"Ocytocine (Syntocinon®)","classe":["Hormone","Ocytocique","Utérotonique"],"indications":["Déclenchement du travail","Accélération d'un travail hypodynamique","Prévention et traitement de l'hémorragie du post-partum","Délivrance dirigée"],"posologie":"IV IVSE déclenchement : débuter à 2-4 mUI/min, augmenter par paliers de 2 mUI/min toutes les 30 min (max 20 mUI/min) · HPP : 5-10 UI IVL lent puis 10-40 UI/h IVSE","contre_indications":["Présentation dystocique non corrigée","Placenta prævia","Disproportion fœto-pelvienne","Cicatrice utérine (relative)"],"effets_secondaires":["Hypercinésie utérine (trop de CU) → SFA","Hypotension transitoire (effet vasodilatatoire en IVL rapide)","Hyponatrémie (effet antidiurétique à fortes doses)","Rupture utérine (si hypercinésie sur cicatrice)"],"service":["Maternité","Bloc obstétrical","Urgences gynécologiques"],"mécanisme":"Se fixe sur les récepteurs à l'ocytocine du myomètre → augmentation du Ca²⁺ intracellulaire → contraction des fibres musculaires lisses utérines. Les récepteurs augmentent en fin de grossesse.","interactions":["Prostaglandines → potentialisation de l'effet utérotonique → association CI ou très prudente","Anesthésiques halogénés → diminuent la réponse utérine"],"niveau_preuve":"A","tags":["médicament","hormone","maternité","utérotonique","obstétrique"],"revision":"2026-06-14"}}
---


> [!abstract] En 2 mots
> Hormone utérotonique naturelle reproduite synthétiquement. Déclenche et régule les contractions utérines. Indispensable en obstétrique pour le déclenchement du travail et la prévention/traitement de l'hémorragie du post-partum. Risque majeur : hypercinésie → souffrance fœtale aiguë.

---

## 💊 À quoi ça sert ?

**Déclenchement du travail** (col mûr) : en IVSE progressive, augmentation par paliers selon le protocole du service jusqu'à obtenir 3-5 CU efficaces en 10 min.

**Hémorragie du post-partum** : bolus 5-10 UI IV lent à la délivrance du placenta (prévention) + IVSE 20-40 UI/h si saignement actif.

**Délivrance dirigée** : injection systématique à la naissance de l'épaule antérieure pour réduire le risque d'HPP.

---

## 🔬 Comment ça marche ? (simplifié)

L'ocytocine imite l'hormone naturelle produite par l'hypothalamus → se fixe sur les récepteurs du muscle utérin (qui multiplient en fin de grossesse) → contractions rythmiques du myomètre.

> 💡 Analogie : l'ocytocine est comme un chef d'orchestre qui donne le tempo aux muscles de l'utérus. En IVSE, on règle le volume (la dose) pour obtenir le bon rythme de contractions.

---

## ⚠️ Ce qu'il ne faut PAS faire

> [!danger] Hypercinésie utérine → souffrance fœtale aiguë
> Plus de 5 CU en 10 min ou CU > 90 secondes → réduction du débit utéro-placentaire → SFA. **STOP l'ocytocine immédiatement** + position latérale gauche + O₂ + appel sage-femme + tocolyse si prescrite (salbutamol, ritodrine).

> [!danger] Ne jamais injecter en IVL rapide à fortes doses
> L'ocytocine à fortes doses en bolus rapide → hypotension brutale → malaise maternel. La dose prophylactique (5-10 UI) doit être injectée lentement en 1-2 min minimum.

> [!warning] Surveillance du tracé CTG obligatoire
> Toute perfusion d'ocytocine → monitoring fœtal continu (CTG) → surveiller la FCF et la fréquence des CU en permanence.

---

## 🩺 Ce que je fais en tant qu'IDE/sage-femme

- [ ] **CTG en continu** dès le début de la perfusion jusqu'à l'accouchement
- [ ] Débuter au débit minimal prescrit et augmenter par paliers selon protocole
- [ ] Surveiller : nombre de CU, durée des CU, FCF entre les CU
- [ ] Si hypercinésie → STOP ocytocine, position latérale gauche, O₂, appel immédiat
- [ ] Surveiller la TA maternelle toutes les 30 min
- [ ] Bilan des entrées/sorties (effet antidiurétique possible à hautes doses)
- [ ] Tracer le débit dans le dossier à chaque modification

---

## 📌 Les 3 choses à retenir absolument

1. **CTG continu obligatoire** pendant toute perfusion d'ocytocine
2. **Hypercinésie → STOP immédiat** + appel sage-femme
3. **HPP prophylaxie** : 5-10 UI IV lent à la naissance de l'épaule antérieure = standard de soins

---

## 🔗 Voir aussi

[[Pathologies/Hémorragie post-partum\|HPP]] · [[Soins Infirmiers/Surveillance obstétricale\|Surveillance obstétricale]] · [[Pathologies/Menace accouchement prémature\|MAP]]

> [!caution] Outil pédagogique : Toujours vérifier le protocole du service et les prescriptions médicales
