---
{"dg-publish":true,"permalink":"/medicaments/hormones/testosterone/","tags":["médicament","hormone","androgène","hypogonadisme","polyglobulie","gel_transfert","fertilité"],"noteIcon":"","dg-note-properties":{"nom":"Testostérone","classe":["Hormone androgène","Stéroïde anabolisant"],"indications":["Hypogonadisme masculin (déficit androgène → 1ère ligne)","Retard pubertaire","Cancer du sein métastatique hormonodépendant (hors AMM → historique)","Femmes ménopausées avec dysfonction sexuelle (hors AMM)"],"posologie":"Gel 50 mg/j (Androgel® 1 sachet/j) · IM : Sustanon® 250 mg/4 sem · IM dépôt : Nébido® 1000 mg/12 sem · Timbre transdermique : Androderm® 5 mg/j","contre_indications":["Cancer de la prostate ou sein connu ou suspecté","Polyglobulie (hématocrite > 54%)","Apnée du sommeil sévère non traitée","Insuffisance cardiaque sévère","Grossesse"],"effets_secondaires":["Polyglobulie (érythrocytose → risque thrombose)","Acné, séborrhée","Alopécie androgénétique","Gynécomastie (aromatisation → œstradiol)","↑ Risque cardiovasculaire","Réduction spermatogenèse (azoospermie si fortes doses)","Transfert gel cutané à l'entourage (femmes enceintes, enfants)"],"service":["Endocrinologie","Urologie","Andrologie"],"mécanisme":"Liaison aux récepteurs aux androgènes (AR) nucléaires. Partiellement convertie en DHT (hypertrophie prostatique, alopécie) et en œstradiol (gynécomastie). Supprime LH et FSH par rétro-inhibition → inhibition spermatogenèse endogène.","interactions":["Warfarine : ↑ effet anticoagulant (surveiller INR)","Insuline/ADO : ↑ sensibilité à l'insuline → risque hypoglycémie","Corticoïdes : ↑ effets anabolisants et rétention hydrosodée"],"niveau_preuve":"A","tags":["médicament","hormone","androgène","hypogonadisme","polyglobulie","gel_transfert","fertilité"],"revision":"2026-06-22"}}
---


<div class="nursekit-nav" style="display:flex;gap:10px;margin-bottom:2rem;padding-bottom:1.5rem;border-bottom:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
</div>
> [!abstract] En 2 mots
> Le carburant hormonal du système masculin → on le prescrit quand les testicules ne fournissent plus assez. Deux points à ne jamais oublier : surveiller la NFS (risque de polyglobulie qui peut mener à la thrombose) et alerter sur le transfert du gel à l'entourage, notamment les femmes enceintes et les enfants.

## À quoi ça sert ?

- Traiter le déficit en testostérone chez l'homme (hypogonadisme primaire ou secondaire)
- Retard pubertaire masculin
- Selon la forme : gel quotidien, injection IM mensuelle ou trimestrielle (Nébido® = une injection tous les 3 mois, pratique pour l'observance)

## Comment ça marche ? (simplifié)

La testostérone remplace ce que les testicules ne produisent plus en quantité suffisante. Elle agit sur beaucoup de cibles : muscles, os, libido, globules rouges, spermatogenèse, caractères sexuels secondaires. Mais attention → une partie de la testostérone est convertie en œstradiol (d'où la gynécomastie possible) et en DHT (d'où l'impact sur la prostate et les cheveux).

> Image mémo : la testostérone, c'est le carburant hormonal du système reproducteur masculin. Quand le réservoir est vide, tout tourne au ralenti → énergie, libido, musculature, humeur. Mais trop de carburant peut aussi engorger le moteur (polyglobulie, prostate).

## Ce qu'il ne faut PAS faire

> [!danger] CI absolues
> - Cancer de la prostate connu ou suspecté → CI formelle absolue
> - Polyglobulie (hématocrite > 54%) → risque de thrombose majeur
> - Apnée du sommeil sévère non traitée → aggravation possible

> [!warning] Interactions majeures
> - Warfarine → ↑ effet anticoagulant (surveiller l'INR)
> - Insuline / ADO → risque d'hypoglycémie par ↑ sensibilité à l'insuline

> [!caution] À surveiller
> - **Érythrocytose (polyglobulie)** : NFS + hématocrite réguliers → le vrai danger caché de ce traitement
> - **PSA** : dépistage prostatique avant de commencer et surveillance annuelle
> - **Transfert du gel** : zones appliquées doivent sécher avant tout contact → laver les mains systématiquement
> - Acné, changements d'humeur, apnée du sommeil à signaler

## Ce que je fais en tant qu'IDE

- [ ] Gel : appliquer sur épaules, bras ou abdomen → PAS les organes génitaux → laisser sécher 3 à 5 min
- [ ] Avertir de ne pas laisser l'entourage (femmes enceintes, enfants) toucher les zones avant lavage
- [ ] Se laver les mains après chaque application → ce n'est pas facultatif
- [ ] Surveiller NFS et hématocrite (polyglobulie → risque de thrombose)
- [ ] Vérifier PSA avant instauration et tous les ans
- [ ] Injection IM (Androtardyl®/Nébido®) : tracer date, dose, site d'injection dans le dossier

## Les 3 choses à retenir

1. **Surveiller NFS + hématocrite** → l'érythrocytose peut passer inaperçue et provoquer une thrombose
2. **Gel transférable à l'entourage** → consignes de contact strictes, systématiquement expliquées
3. **PSA obligatoire** avant et pendant le traitement → prostate à surveiller toute la durée

## Voir aussi

[[Médicaments/Hormones/Oestrogene\|Œstrogènes]] · NFS · Hématocrite

> [!caution] Outil pédagogique : Toujours vérifier le RCP et les protocoles du service

<div class="nursekit-nav" style="display:flex;gap:10px;margin-top:2.5rem;padding-top:1.5rem;border-top:1px solid rgba(99,102,241,0.2);">
<a href="/" style="flex:1;display:block;text-align:center;padding:10px 16px;background:#6366F1;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;">← Accueil</a>
<a href="#" style="flex:1;display:block;text-align:center;padding:10px 16px;background:rgba(99,102,241,0.08);color:#6366F1;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;border:1px solid rgba(99,102,241,0.25);">↑ Haut de page</a>
</div>
